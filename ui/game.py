...
class WordleGame(QMainWindow):
    ...
    def load_word_list(self):
        ...
        if not self.valid_words:
            # Fallback to hardcoded lists if database fetch fails
            print("Warning: Could not load words from database. Using fallback word list.")
            if self.language == "spanish":
                self.valid_words = ["GATOS", "PERRO", "CASAS", "LIBRO", "VERDE", "AZULE", 
                                   "CIELO", "MUNDO", "FELIZ", "PARAR", "COMER", "ANDAR",
                                   "RELOJ", "PLAYA", "LLAVE", "NEGRO", "BLUSA", "SALSA"]
            else:  # English
                self.valid_words = ["HAPPY", "BREAD", "WATER", "HOUSE", "MUSIC", "TIGER", 
                                   "GAMES", "PLANE", "WATCH", "PIXEL", "PLANT", "DRINK",
                                   "MOVIE", "WHEEL", "LIGHT", "STORM", "APPLE", "OCEAN"]
