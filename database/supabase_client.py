...
def sign_in(username: str, password: str) -> dict:
    ...
    # Get user by username and join with tipo_usuario to get admin status
    # Adjust the select query if your Supabase client library has a different way to do joins or select specific columns from joined tables.
    # This is a conceptual representation. The actual Supabase query might need to be structured differently, 
    # possibly using a view or an RPC function if direct join syntax in select isn't straightforward.
    # For now, we'll fetch from 'usuarios' and then separately query 'tipo_usuario' or assume 'tipo_usuario_id' directly maps to admin status if simple.

    user_result = client.table("usuarios").select("id, nombre_usuario, contrasena, email, tipo_usuario_id").eq("nombre_usuario", username).execute()
...
# Game Data Functions
def save_game_result(user_id: int, word: str, language: str, attempts: int, time_taken: float, win: bool, hints_used: int):
...
def get_language_distribution() -> list:
    ...
    client = get_supabase_client()
    # This requires joining 'partidas' with 'palabras' then 'idiomas' and grouping by 'idiomas.idioma'.
    # This might be best done with an SQL view or an RPC function in Supabase for simplicity here.
    # For example, an RPC function 'get_language_game_counts()'.
    # A simplified version for now, assuming direct query capabilities or that the UI can process raw IDs:
    # This is a conceptual query. The actual implementation might require an RPC call for this aggregation.
    # result = client.rpc('get_language_distribution_stats').execute()
    # Fallback: Fetch all partidas and process in Python, or adjust if your client supports complex joins for aggregation.
    # The direct Supabase query for this is non-trivial with group by on a joined table's column.
    # Let's assume we have an RPC or will process this client-side for now.
    # As a placeholder, returning raw language IDs from 'palabras' referenced in 'partidas':
    # This is NOT the final desired output but a step if direct complex query is not used.
    # A proper solution would be an RPC: CREATE FUNCTION get_lang_distro() RETURNS TABLE(language_name TEXT, game_count BIGINT) ...
    # For now, we'll return something that the admin panel might need to further process or we can refine this with an RPC call.
    # Since we don't have an RPC function, we'll implement this in Python by fetching the necessary data and processing it.
    try:
        ...
        idiomas_result = client.table("idiomas").select("id, idioma").in_("id", list(language_distribution.keys())).execute()
        ...
        print(f"Error calling RPC get_language_stats: {e}. Returning empty list. You may need to create this RPC function in Supabase.")
        return []
        ...
        if language_name is None:
            ...
            for palabra_id in sorted_palabra_ids:
                ...
                if palabra_result.data and len(palabra_result.data) > 0:
                    ...
                    # Get idioma info
                    idioma_result = client.table("idiomas").select("idioma").eq("id", palabra_info["idioma_id"]).execute()
            ...
            partidas_result = client.table("partidas").select("palabra_id, intentos").in_("palabra_id", palabra_ids).execute()
            partidas = partidas_result.data if partidas_result.data else []
